...
// DELIVER PRODUCT
export async function updateMeatSelectionStatus(serial_no : string, status : string) {
    await pool.query(`
        UPDATE meat_selection
        SET status = ?
        WHERE serial_no = ?;
...
export async function updatePendingDeliveryStatus(deliveryNo : number, distanceTraveled : number, deliveryDuration : number, status : string) {
    await pool.query(`
        UPDATE deliveries
        SET 
            distance_traveled = ?,
...
export async function updatePendingDeliveryInfo(delivery_no : string, driverName : string, truckNumber : number) {
    await pool.query(`
        UPDATE deliveries
        SET 
            driver_name = ?
